...
describe("doc", () => {
  ...
  test("a 자리비움 (hidden-tab) member doesn't block the requester's exclude", async () => {
    spyOn(SessionPrompt, "prompt").mockImplementation(() => Promise.resolve(undefined as never))
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      ...
      fn: async () => {
        ...
        // Bob's tab is visible: he is a live decision-maker, so exclude must be a no-op.
        const blocked = Doc.submitRespond({
          sessionID: session.id,
          submitID: state.submitID,
          actorID: alice.actorID,
          action: "exclude",
        ...
        // Bob's tab goes hidden (자리비움): every dialog learns via an updated cast...
        stopBob.visibility(true)
        const marked = aliceEvents.findLast((event) => event.type === "updated")
        expect(marked?.state.actors.find((actor) => actor.actorID === bob.actorID)?.away).toBe(true)

        // ...and the requester may now proceed without him — even with notifications blocked,
        // nothing in this path depends on bob ever seeing one.
        const sent = Doc.submitRespond({
          sessionID: session.id,
          submitID: state.submitID,
          actorID: alice.actorID,
  ...
  test("coming back to a visible tab clears 자리비움 and blocks exclude again", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      init: InstanceBootstrap,
      fn: async () => {
        ...
        expect(held.status).toBe("pending")
        expect(held.actors.find((actor) => actor.actorID === bob.actorID)?.away).toBeUndefined()
  ...
  test("submit approval clamps timeout and auto-approves initiator", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(base.timeoutMs).toBe(30_000)
        stop()
      },
    })
  })
