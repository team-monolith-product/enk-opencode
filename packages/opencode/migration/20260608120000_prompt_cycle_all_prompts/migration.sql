-- Rework prompt_cycle to record EVERY prompt→response turn (normal/shell/doc), not just
-- doc-consent submits. doc_id/actor/consent become nullable; the cycle is keyed by the
-- assistant message. No data to preserve (feature unreleased), so drop & recreate.
