...
export const ProjectSummaryRoutes = lazy(() =>
  new Hono().post(
    ...
    async (c) => {
      ...
      const language = await ModelPolicy.resolveModel(msgs, {
      ...
      const files = await WorkspaceContext.collect(session.directory)
      const workspaceContext = await WorkspaceContext.render(session.directory, files, transcript)
